...
public class AmplifyColorEffect : MonoBehaviour
{
	...
	{
		DebugTWD.LogError( "[AmplifyColor] Failed to initialize shaders. Please attempt to re-enable the Amplify Color Effect component. If that fails, please reinstall Amplify Color." );
		enabled = false;
	}

	void ReportNotSupported()
	{
		DebugTWD.LogError( "[AmplifyColor] This image effect is not supported on this platform." );
		enabled = false;
	}
	...
	void OnEnable()
	{
		...
		if ( nullDev )
		{
			DebugTWD.LogWarning( "[AmplifyColor] Null graphics device detected. Skipping effect silently." );
			silentError = true;
			return;
		}
		...
		if ( ( lutTex2d != null && lutTex2d.mipmapCount > 1 ) || ( lutBlendTex2d != null && lutBlendTex2d.mipmapCount > 1 ) )
			DebugTWD.LogError( "[AmplifyColor] Please disable \"Generate Mip Maps\" import settings on all LUT textures to avoid visual glitches. " +
	...
	bool CheckMaterialAndShader( Material material, string name )
	{
		...
		{
			DebugTWD.LogWarning( "[AmplifyColor] Error creating " + name + " material. Effect disabled." );
			enabled = false;
		}
		else if ( !material.shader.isSupported )
		{
			DebugTWD.LogWarning( "[AmplifyColor] " + name + " shader not supported on this platform. Effect disabled." );
			enabled = false;
		}
		else
		{
	...
	public static bool ValidateLutDimensions( Texture lut )
	{
		...
		if ( lut != null )
		{
			...
			{
				DebugTWD.LogWarning( "[AmplifyColor] Lut " + lut.name + " has invalid dimensions." );
				valid = false;
			}
			else
			{
