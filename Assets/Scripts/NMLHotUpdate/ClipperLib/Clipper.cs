	...
	public class Clipper : ClipperBase
	{
		...
		private void ProcessIntersectList()
		{
			for (int i = 0; i < m_IntersectList.Count; i++)
			{
				...
				IntersectEdges(intersectNode.Edge1, intersectNode.Edge2, intersectNode.Pt, protect: true);
		...
		private int PointInPolygon(IntPoint pt, OutPt op)
		{
			...
			do
			{
				double num2 = op.Pt.X;
				double num3 = op.Pt.Y;
				double num4 = op.Next.Pt.X;
				double num5 = op.Next.Pt.Y;
				if (num5 == (double)pt.Y && (num4 == (double)pt.X || (num3 == (double)pt.Y && num4 > (double)pt.X == num2 < (double)pt.X)))
				{
					return -1;
				}
				if (num3 < (double)pt.Y != num5 < (double)pt.Y)
				{
					if (num2 >= (double)pt.X)
					{
						if (num4 > (double)pt.X)
						...
						{
							double num6 = (num2 - (double)pt.X) * (num5 - (double)pt.Y) - (num4 - (double)pt.X) * (num3 - (double)pt.Y);
							if (num6 == 0.0)
							{
								return -1;
							}
					...
					else if (num4 > (double)pt.X)
					{
						double num7 = (num2 - (double)pt.X) * (num5 - (double)pt.Y) - (num4 - (double)pt.X) * (num3 - (double)pt.Y);
						if (num7 == 0.0)
						{
							return -1;
						}
		...
		private void FixupFirstLefts1(OutRec OldOutRec, OutRec NewOutRec)
		{
			...
			{
				OutRec outRec = m_PolyOuts[i];
				if (outRec.Pts != null && outRec.FirstLeft == OldOutRec && Poly2ContainsPoly1(outRec.Pts, NewOutRec.Pts))
				{
				...
				if (outRec == outRec2)
				{
					...
					if (m_UsingPolyTree)
					{
						for (int j = 0; j < m_PolyOuts.Count - 1; j++)
						{
							...
							if (outRec4.Pts != null && ParseFirstLeft(outRec4.FirstLeft) == outRec && outRec4.IsHole != outRec.IsHole && Poly2ContainsPoly1(outRec4.Pts, obj.OutPt2))
							{
