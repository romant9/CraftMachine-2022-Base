...
public class ResourceDeeplinkItem : MonoBehaviour
{
	[SerializeField]
	...
	private CurrencyType linkedCurrency;
	...
	private UIType parentPopupType;

	private Dictionary<TWDModelObject, MethodInfo> modelTimerMethods = new Dictionary<TWDModelObject, MethodInfo>();

	public void SetParameters(CurrencyType resourceCurrency, AvailableRewardsCollector collectorIn, TWDModelObject objectToInspect, string deeplinkName, string deeplinkURL, string refreshLocKey, bool timerBased = false, UIType parentPopupType = UIType.None)
	{
		linkedCurrency = resourceCurrency;
		collector = collectorIn;
		twdModelObject = objectToInspect;
		this.deeplinkName = deeplinkName;
		this.deeplinkURL = deeplinkURL;
		this.timerBased = timerBased;
		this.parentPopupType = parentPopupType;
	...
	public void UpdateUI()
	{
		...
		bool flag = solver.HasRewardsOfType(linkedCurrency, collector.GetRewards());
		if (timerBased)
		{
			if (flag)
			{
		...
		{
			Helpers.GameObjectSetActive(buttonParent, flag);
		}
	...
	public void OnClickGo()
	{
		...
		if (!string.IsNullOrEmpty(text))
		{
			...
			{
				SingularityMonoBehaviour<HUDManager>.Instance.CloseIfExists(parentPopupType);
			}
		}
	}
