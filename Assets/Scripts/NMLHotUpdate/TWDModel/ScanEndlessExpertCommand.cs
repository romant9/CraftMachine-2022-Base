...
namespace TWDModel
{
	public class ScanEndlessExpertCommand : ModelCommand
	{
		[JsonIgnore]
		...
		public override IModelCommandRespond Execute(ModelManager manager)
		{
			if (manager is TWDModelManager tWDModelManager)
			{
				...
				if (endlessModeManager == null || !endlessModeManager.CheckCanScanExpert())
				{
					return new NGModelCommandRespond(this, TWDModelResult.Error);
				}
				int missionBaseCost = tWDModelManager.GameEconomyData.EndlessModeConfig.MissionBaseCost;
				Cashier cashier = Cashier.CreateOneItemCashier(tWDModelManager, PurchaseType.EndlessPass, CurrencyType.EndlessPassExpertToken, missionBaseCost);
