...
namespace TWDModel
{
	public class RouletteMultiDrawCommand : ConsumeCurrencyCommand
	{
		[JsonIgnore]
		...
		public RouletteMultiDrawCommand()
		{
		}

		public RouletteMultiDrawCommand(int rouletteConfigId)
		{
		...
		public override IModelCommandRespond Execute(ModelManager manager)
		{
			if (manager is TWDModelManager { Player: var player } tWDModelManager)
			{
				...
				{
					manager?.Debug?.LogWarning($"RouletteMultiDrawCommand: Draw not allowed for activity {RouletteConfigId} - activity may be completed");
					return new NGModelCommandRespond(this, TWDModelResult.Error);
				}
				if (activityData.LastDrawTime > 0)
				{
					...
					if (num < 3000)
					{
						manager?.Debug?.LogWarning($"RouletteMultiDrawCommand: Draw too fast for activity {RouletteConfigId}, time since last draw: {num}ms");
						return new NGModelCommandRespond(this, TWDModelResult.Error);
					}
				}
				Cashier cashier = RouletteDrawCommand.GetCashier(tWDModelManager, RouletteConfigId, isMultiDraw: true);
				if (cashier == null)
				{
					return new NGModelCommandRespond(this, TWDModelResult.Error);
				}
				...
				RouletteResult rouletteResult = activityData?.ExecuteMultiDraw();
				...
				if (rouletteResult?.GetAllRewardsList() == null)
				{
					new List<RouletteDefinition>();
				}
				bool flag = rouletteResult?.HasGetAllReward ?? false;
				if (activityData != null && activityData.IsActivityCompleted())
				{
					tWDModelManager.TdMetrics.SetEventType("roulette_activity_completed").AddProperty("config_id", RouletteConfigId.ToString()).AddProperty("multi_draw", "true")
						.AddProperty("has_get_all", flag.ToString())
						.Send();
				}
				Rewards grantedRewards = null;
				if (activityData == null || !activityData.GrantRouletteRewards(rouletteResult, out grantedRewards))
				{
					tWDModelManager.Debug.LogWarning($"RouletteMultiDrawCommand: Reward granting failed for config {RouletteConfigId}, recorded for retry");
				}
				Rewards = grantedRewards;
				Metrics metrics = tWDModelManager.Metrics;
				metrics.ResourceChangeUsedReason = "RouletteMultiDraw";
				metrics.AddItemChange().AddResources(cashier).Send();
				RouletteConfig config = activityData?.GetConfig();
				SendRouletteRollMetrics(tWDModelManager, activityData, rouletteResult, config, isMultiDraw: true);
				return new NGModelCommandRespond(this, TWDModelResult.OK);
			}
			return new NGModelCommandRespond(this, TWDModelResult.Error);
		}

		private static void SendRouletteRollMetrics(TWDModelManager manager, RouletteActivityDataModel activityData, RouletteResult rouletteResult, RouletteConfig config, bool isMultiDraw)
		{
			try
			{
				...
				Dictionary<CurrencyType, int> multiCostInfo = config.GetMultiCostInfo();
				string value = "";
				int num = 0;
				if (multiCostInfo != null && multiCostInfo.Count > 0)
				{
					KeyValuePair<CurrencyType, int> keyValuePair = multiCostInfo.First();
