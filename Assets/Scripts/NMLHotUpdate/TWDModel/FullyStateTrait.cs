...
namespace TWDModel
{
	public class FullyStateTrait : ActionModifier
	{
		public override ActionListClearFlag VisitActions(ModelAction action, ActorModel actor, List<ModelAction> addedActions)
		{
			TraitEntry trait = actor.TraitContainer.GetTrait("TemFullyStateTrait");
			if (trait == null)
			{
			...
			else if (actor.manager.CombatModel.TurnManager.ActiveFaction == actor.Faction)
			{
				trait.TraitDuration--;
				if (trait.TraitDuration <= 0)
				{
					actor.RemoveTrait("TemFullyStateTrait");
				}
			}
			return ActionListClearFlag.Keep;
		}
