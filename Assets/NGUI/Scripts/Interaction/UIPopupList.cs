...
public class UIPopupList : UIWidgetContainer
{
	...
	public UILabel.Effect textEffect;


	public bool separatePanel = true;

	[SerializeField]
	public static bool ItemIsGameobject = false;
	...
	public virtual void RemoveItem(string text)
	{
		...
		{
			items.RemoveAt(num);
			if (itemData.Count > num) itemData.RemoveAt(num);
	...
	protected virtual void AnimatePosition(UIWidget widget, bool placeAbove, float bottom)
	{
		...
		Vector3 localPosition2 = (placeAbove ? new Vector3(localPosition.x, bottom, localPosition.z) : new Vector3(localPosition.x, NGUIManager.IsLoadDataManager ? 10f : 0f, localPosition.z));
	...
	public virtual void Show()
	{
		if (base.enabled && NGUITools.GetActive(base.gameObject) && mChild == null && isValid && items.Count > 0)
		{
			mLabelList.Clear();
			StopCoroutine("CloseIfUnselected");
			UICamera.selectedObject = ((UICamera.hoveredObject != null) ? UICamera.hoveredObject : base.gameObject);
			...
			for (int count = items.Count; i < count; i++)
			{
				...
				uILabel.font = bitmapFont as INGUIFont;
				...
				uILabel.effectStyle = textEffect;
				...
				uILabel.symbolStyle = NGUIText.SymbolStyle.Normal;
				list.Add(uILabel);
				num8 += num6;
				num7 -= num6;
			...
			if (flag)
			{
				...
				transform.localPosition = new Vector3(vector.x, vector2.y - num11 + (NGUIManager.IsLoadDataManager ? 10f : 0f), vector.z);
			}
			else
			{
