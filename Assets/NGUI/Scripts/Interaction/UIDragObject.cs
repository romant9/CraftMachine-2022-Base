...
public class UIDragObject : MonoBehaviour
{
	...
	private void OnScroll(float delta)
	{
		if (base.enabled && NGUITools.GetActive(base.gameObject))
		{
			if (IsZoomAvailable)
			{
                target.localScale += scrollMomentum * (delta * 0.05f);
            }
			else
			{
        ...
        if (Input.touchCount > 0)
        {
            if (base.enabled && NGUITools.GetActive(base.gameObject))
            {
                if (Input.touchCount == 2)
                {
                    ...
                    target.localScale += scrollMomentum * (deltaMagnitudeDiff * 0.05f) * .1f;
                    var scaleX = Mathf.Clamp(target.localScale.x, .8f, 2.2f);
                    var scaleY = Mathf.Clamp(target.localScale.y, .8f, 2.2f);

                    target.localScale = new Vector2(scaleX, scaleY);
                }
            }
        }
    }
