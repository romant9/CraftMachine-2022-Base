...
public class UIButton : UIButtonColor
{
    ...
    public List<EventDelegate> onDragFinish = new List<EventDelegate>();
	...
	public override bool isEnabled
	{
		...
		set
		{
			...
			if (component != null)
			{
				...
				UIButtonColor[] components = GetComponents<UIButtonColor>();
				for (int i = 0; i < components.Length; i++)
				{
					components[i].SetState((!value) ? State.Disabled : State.Normal, instant: false);
				}
				return;
			}
			Collider2D component2 = GetComponent<Collider2D>();
	...
	protected override void OnDragOut()
	{
		if (isEnabled && (dragHighlight || UICamera.currentTouch.pressed == base.gameObject))
		{
            EventDelegate.Execute(onDragFinish);
	...
	{
		if (mIgnoreFrame != Time.frameCount && current == null && isEnabled && UICamera.currentTouchID != -2 && UICamera.currentTouchID != -3)
		{
			current = this;
			mIgnoreFrame = Time.frameCount;
