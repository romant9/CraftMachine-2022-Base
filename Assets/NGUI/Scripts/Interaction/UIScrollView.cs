...
public class UIScrollView : MonoBehaviour
{
	...
	public bool restrictImmediately;

	public bool restrictToCustomBounds;

	public Bounds customBoundsForRestrict;
	...
	public bool IsMooving
	{
		get
		{
            return Math.Abs(mMomentum.magnitude) >= .0001f;
		}
	}
	...
	public bool RestrictWithinBounds(bool instant, bool horizontal, bool vertical)
	{
		...
		Bounds bounds = this.bounds;
		if (restrictToCustomBounds)
		{
			bounds = customBoundsForRestrict;
		}
    ...
    [ContextMenu("Reset Clipping Position Inverse")]
    public void ResetPositionInverted()
    {
        if (NGUITools.GetActive(this))
        {
            mCalculatedBounds = false;
            Vector2 pivotOffset = NGUIMath.GetPivotOffset(contentPivot);
            SetDragAmount(pivotOffset.x, pivotOffset.y, updateScrollbars: false);
            SetDragAmount(pivotOffset.x, pivotOffset.y, updateScrollbars: true);
        }
    }
    public void ResetPositionTo(float y)
    {
        if (NGUITools.GetActive(this))
        {
            mCalculatedBounds = false;
            Vector2 pivotOffset = NGUIMath.GetPivotOffset(contentPivot);
            SetDragAmount(pivotOffset.x, y, updateScrollbars: false);
            SetDragAmount(pivotOffset.x, y, updateScrollbars: true);
        }
    }

    [ContextMenu("Update Clipping Position")]
