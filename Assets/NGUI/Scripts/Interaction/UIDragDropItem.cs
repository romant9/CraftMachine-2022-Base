...
public class UIDragDropItem : MonoBehaviour
{
	...
	{
		if (interactable && mIgnoreClick != Time.frameCount && clickToDrag && !mDragging && UICamera.currentTouchID == -1 && draggedItems.Count == 0)
		{
			mTouch = UICamera.currentTouch;
	...
	protected virtual void OnDragDropMove(Vector2 delta)
	{
		if (mParent != null)
		{
			Vector3 vector = mTrans.InverseTransformDirection(delta);
			if (mRoot != null)
			{
				vector = mRoot.transform.TransformDirection(vector);
			}
