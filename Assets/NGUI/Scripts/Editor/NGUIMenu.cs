...
static public class NGUIMenu
{
	...
	static void CheckForIssues (Transform t)
	{
		...
		{
			Debug.LogWarning(NGUITools.GetHierarchy(t.gameObject) + " has a large position offset and is likely to cause floating precision issues");
		}

		if (t.GetComponent<UIRoot>() == null && t.localScale != Vector3.one)
		{
			Debug.LogWarning(NGUITools.GetHierarchy(t.gameObject) + " doesn't have a uniform scale. Consider changing to (1, 1, 1).");
		}
		...
		{
			Debug.LogWarning(NGUITools.GetHierarchy(t.gameObject) + " has a panel with a large clipping offset. Consider resetting it to zero.");
		}
