...
public class UIAtlasMaker : EditorWindow
{
			...
			if (newTexture)
			{
				...
				else
				{
					...
					if (mat == null)
					{
						...
						var shader = NGUISettings.atlasPMA ? NGUIManager.PremultColoredShader : NGUIManager.TranspColoredShader;
	...
	static public void UpdateAtlas (INGUIAtlas obj, List<SpriteEntry> sprites)
	{
		...
		{
			Debug.LogWarning("Updating a legacy atlas: issues may occur. Please update the atlas to a new format that uses Scriptable Objects rather than Prefabs.", context: obj as UnityEngine.Object);
