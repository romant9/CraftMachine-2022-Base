...
static public class FreeType
{
	...
	static public bool CreateFont (Font ttf, int size, int faceIndex, bool kerning, string characters, int padding, out BMFont font, out Texture2D tex)
	{
		...
		else
		{
			...
			if (kerning)
			{
				if (kerningData == 0) Debug.LogWarning("The font doesn't appear to have any kerning information for the chosen characters", context: ttf);
				else Debug.Log("Kerning data: " + kerningData + " entries");
			}
	...
	static public List<NGUIFont.KerningAdjustment> GetKerning (Font ttf, int size, int faceIndex, string characters)
	{
		...
		else
		{
			...
			if (list.Count == 0) Debug.LogWarning("The font doesn't appear to have any kerning information for the chosen characters. You will have to edit each combination manually.", context: ttf);
			else Debug.Log("Kerning data: " + list.Count + " entries");
		}
