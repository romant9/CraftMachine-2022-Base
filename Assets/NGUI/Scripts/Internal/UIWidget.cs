...
public class UIWidget : UIRect
{
	...
	//[HideInInspector]
	[SerializeField] protected float mColorMult = 1f;
	//[HideInInspector]
	[SerializeField] protected Color mColor = Color.white;
	//[HideInInspector]
	[SerializeField] protected Pivot mPivot = Pivot.Center;
	//[HideInInspector]
	[SerializeField] protected int mWidth = 100;
	//[HideInInspector]
	[SerializeField] protected int mHeight = 100;
	//[HideInInspector]
	[SerializeField] protected int mDepth = 0;
	...
	public delegate void OnAnchorDimensionsChanged();

	public OnAnchorDimensionsChanged OnAnchorDimensionsChangedChange;
	...
	public virtual Shader shader
	{
		get
		{
			...
			return NGUIManager.GetWidgetShader(mat);
		}
		set
		{
...
//#endif
	...
	protected override void OnStart()
	{
		...
		{
			Debug.LogError("Widgets and panels should not be on the same object! Widget must be a child of the panel. " + this.name, context: this);
		}
		else if (!Application.isPlaying && GetComponents<UIWidget>().Length > 1)
		{
			Debug.LogError("You should not place more than one widget on the same object. Weird stuff will happen!", context: this);
		}
#endif
		CreatePanel();
	}
	...
	protected override void OnAnchor()
	{
		...
		if (mWidth != w || mHeight != h)
		{
			...
			if (mIsInFront) mChanged = true;
			if (autoResizeBoxCollider) ResizeCollider();
			if (OnAnchorDimensionsChangedChange != null) OnAnchorDimensionsChangedChange();
		}
	}
...
//#if UNITY_EDITOR
	...
	static public bool showHandlesWithMoveTool
	{
		get
		{
			if (mHandles == -1 && !Application.isPlaying)
			{
				//mHandles = UnityEditor.EditorPrefs.GetInt("NGUI Handles", 1);
				mHandles = NGUIManager.GetMHandles();
			}
			return mHandles == 1;
		}
		set
		{
			...
			if (mHandles != val)
			{
				...
				NGUIManager.SetMHandles(mHandles);
			}
		}
	}
	...
	static public bool showHandles
	{
		get
		{
			...
			return NGUIManager.IsShowHandles();
		}
	}
	...
	protected void OnDrawGizmos()
	{
		if (isVisible && isSelectable && NGUITools.GetActive(this))
		{
			...
			if (NGUIManager.IsActiveGameobject(gameObject) && showHandles) return;
			...
			Gizmos.color = NGUIManager.IsActiveGameobject(gameObject) ? Color.white : outline;
