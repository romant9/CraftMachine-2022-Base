...
public static class Localization
{
	...
	public static string LocalizationName => NGUIManager.LocalizationName;
	...
	{
		if (NGUIManager.IsSingleLanguageLoad)
		{
			byte[] array = null;
			if (!localizationHasBeenSet)
			{
				...
				{
					TextAsset textAsset = Resources.Load<TextAsset>(LocalizationName);
					if (textAsset != null)
					{
				...
				{
					array = loadFunction(LocalizationName);
				}
		...
		else
		{
			...
			if (!localizationHasBeenSet)
			{
				if (loadFunction == null)
				{
					var assets = Resources.LoadAll<TextAsset>(LocalizationName);
...
#if TNET
					bytes = TNet.Tools.ReadFile(LocalizationName + ".txt") ?? TNet.Tools.ReadFile(LocalizationName + ".csv");
#endif
				}
				else bytes = loadFunction(LocalizationName);
				localizationHasBeenSet = true;
			}
	...
	private static bool LoadCSV(byte[] bytes, TextAsset asset, bool merge = false)
	{
		...
		if (betterList == null)
		{
			if (asset != null)
			{
				Debug.LogError("Unable to parse " + asset.name + " as a CSV file", context: asset);
			}
			else
			{
				Debug.LogError("Unable to parse the specified data as a CSV file");
			}
			return false;
		}
	...
	public static string Get(string key, bool warnIfMissing = true)
	{
		...
		if (mLanguages == null)
		{
			Debug.LogWarning("No localization data present");
			return key;
		}
		string text = language;
		if (mLanguageIndex == -1)
