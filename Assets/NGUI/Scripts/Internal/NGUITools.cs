...
//	  }
	...
	static public Camera FindCameraForLayer (int layer)
	{
		...
		Debug.Log("FindCameraForLayer is null: ");
		return null;
	}
	...
	static public void AddWidgetCollider (GameObject go, bool considerInactive)
	{
		if (go != null)
		{
			...
			if (ui != null && (ui.eventType == UICamera.EventType.World_2D || ui.eventType == UICamera.EventType.UI_2D))
			{
				...
				box2.isTrigger = true;
//#if UNITY_EDITOR
				if (NGUIManager.IsEditor) NGUIManager.RegisterCreatedObjectUndo2D(box2);
...
//#if UNITY_EDITOR
				if (NGUIManager.IsEditor) NGUIManager.RegisterCreatedObjectUndo(box);
...
//#endif
	...
	{
		if (!NGUIManager.IsEditor) return;
...
//#endif
//		if (prefabStage == null)
		if (NGUIManager.IsPrefabStage(gameObject)) return;
		...
		if (!missingRoot && !missingPanel) return;
		...
		var container = NGUIManager.GetContainer();
		...
		NGUIManager.MoveGameObjectToScene(container);
...
//#endif
		NGUIManager.RegisterCreatedObjectUndo(undo, go);
		if (parent != null)
		{
...
//#endif
		NGUIManager.RegisterCreatedObjectUndo(true, go);
		if (go != null)
		{
			var t = go.transform;
			go.name = prefab.name;
			...
			if (parent != null)
			{
				int layerSet = go.layer;
				if (layer == -1) layerSet = parent.layer;
				else if (layer > -1 && layer < 32) layerSet = layer;
				SetLayer(go, layerSet);
			}
...
//#endif
			if (!Application.isPlaying) NGUIManager.SetSelection(panel.gameObject);
...
//#endif
	...
	{
		//if (NGUIManager.IsEditor) return NGUIManager.GetSidesMy(cam, Mathf.Lerp(cam.nearClipPlane, cam.farClipPlane, 0.5f), null);
	...
	{
		//if (NGUIManager.IsEditor) return NGUIManager.GetSidesMy(cam, depth, null);
	...
	{
		//if (NGUIManager.IsEditor) return NGUIManager.GetSidesMy(cam, Mathf.Lerp(cam.nearClipPlane, cam.farClipPlane, 0.5f), relativeTo);
...
//#if UNITY_EDITOR
	...
	public static Vector2 screenSizeEditor
	{
		get
		{
			...
			//Debug.Log("ScreenSizeEditor: " + mGameSize);
			return mGameSize;
		}
	}
...
//#else
	...
	//static public Vector2 screenSize { get { return NGUIManager.ScreenSize(); } }
//#endif
