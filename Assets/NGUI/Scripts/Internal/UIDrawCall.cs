...
//#if UNITY_EDITOR
					if (NGUIManager.IsEditor) if (mRenderer != null) mRenderer.enabled = isActive;
//#endif
				}
			}
		}
...
//#if UNITY_EDITOR
					if (NGUIManager.IsEditor) NGUITools.SetDirty(gameObject);
//#endif
				}
			}
		}
	...
	void CreateMaterial ()
	{
		...
		if (shader == null) shader = NGUIManager.TranspColoredShader;
...
//#if UNITY_EDITOR
				if (NGUIManager.IsEditor) mRenderer.enabled = isActive;
//#endif
#if !UNITY_4_7
				if (mShadowMode == ShadowMode.None)
				{
...
//#endif
	...
	static UIDrawCall Create (string name, UIPanel pan, Material mat, Texture tex, Shader shader)
	{
		...
		if (NGUIManager.IsEditor)
		{
			bool hasManager = dc.manager != null;
			if (hasManager)
			{
				GameObject dcGo = dc.gameObject;
				GameObject dcManagerGo = dc.manager.gameObject;
				NGUIManager.UIDrawCallHelper(hasManager, dcManagerGo, dcGo);
			}
		}
		return dc;
	}
	...
	static UIDrawCall Create (string name)
	{
		...
		if (NGUIManager.IsEditor || NGUIManager.IsEditorRuntime)
		{
		...
		UIDrawCall newDC = gameObject.AddComponent<UIDrawCall>();
