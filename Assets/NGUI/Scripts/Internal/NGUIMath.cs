...
{
	[NonSerialized]
	private static List<UIWidget> s_widgets = new List<UIWidget>();
	...
	public static Bounds CalculateAbsoluteWidgetBounds(Transform trans)
	{
		if (trans != null)
		{
			...
			int i = 0;
			for (int num = s_widgets.Count; i < num; i++)
			{
				UIWidget uIWidget = s_widgets[i];
				if (!uIWidget.isSelectable || !uIWidget.enabled)
				{
			...
			int j = 0;
			for (int count = s_widgets.Count; j < count; j++)
			{
				Vector3[] worldCorners = s_widgets[j].worldCorners;
				for (int k = 0; k < 4; k++)
				{
	...
	private static void CalculateRelativeWidgetBounds(Transform content, bool considerInactive, bool isRoot, ref Matrix4x4 toLocal, ref Vector3 vMin, ref Vector3 vMax, ref bool isSet, bool considerChildren)
	{
		...
		if (component != null && component.enabled && component.isSelectable && !component.boundless)
		{
