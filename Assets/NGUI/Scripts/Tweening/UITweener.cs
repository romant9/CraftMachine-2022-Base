...
public abstract class UITweener : MonoBehaviour
{
	...
	protected void DoUpdate()
	{
		...
		if (!mStarted)
		{
			...
			mStarted = true;
			mStartTime = num2;
			if (mAmountPerDelta > 0f && (delayAffects == DelayAffects.Both || delayAffects == DelayAffects.Forward))
			{
				mStartTime += delay;
			}
			else if (mAmountPerDelta < 0f && (delayAffects == DelayAffects.Both || delayAffects == DelayAffects.Reverse))
			{
				mStartTime += delay;
			}
		}
	...
	public static T Begin<T>(GameObject go, float duration, float delay = 0f) where T : UITweener
	{
		...
		if (val == null)
		{
			...
			if (val == null)
			{
				Debug.LogError("Unable to add " + typeof(T)?.ToString() + " to " + NGUITools.GetHierarchy(go), context: go);
				return null;
			}
		}
		val.mStarted = false;
