...
public class UISprite : UIBasicSprite
{
	...
	public override Material material
	{
		get
		{
			...
			if (mAtlas is INGUIAtlas iNGUIAtlas)
			{
				Material mMat = iNGUIAtlas.spriteMaterial;
				if (mMat != null && (mMat.name.ToLower().Contains("glow") || mMat.name.ToLower().Contains("fx_") || mMat.shader.name.ToLower().Contains("fx_")))
				{
					mMat.SetInt("_ColorToOpacity", 1);
					mMat.SetColor("_Color", color);
				}
	...
	public bool applyHorizontalGradient
	{
		get
		{
			return applyHorizontalGradient;
		}
		set
		{
			if (applyHorizontalGradient != value)
			{
				applyHorizontalGradient = value;
				MarkAsChanged();
			}
		}
	}
