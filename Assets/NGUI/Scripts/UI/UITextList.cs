...
public class UITextList : MonoBehaviour
{
	...
	{
		var lines = paragraphs.ToArray();

		if (lines != null && lines.Length > 0 && lines.Last().lines.Length > 0)
			return string.Join("/n", lines.Last().lines);
		else return "";
    }
    ...
    public string GetAllStrings()
    {
        ...
        if (lines.Length > 0 && lines.Last().lines.Length > 0)
        {
            var strLines = new List<string>();
            foreach (Paragraph line in lines)
            {
                strLines.Add(string.Join("/n", line.lines));
            }
            return strLines.Count > 0 ? string.Join("/n", strLines) : "";
        }
        else return "";
    }
