...
public class UILabel : UIWidget
{
	...
	public bool symbolLabel => mLabSym;
	...
	private bool premultipliedAlphaShader => font?.premultipliedAlphaShader ?? false;

	private bool packedFontShader => font?.packedFontShader ?? false;

	public bool separateSymbols
	{
		get
		{
			NGUIFont nGUIFont = font as NGUIFont;
			if (nGUIFont != null && nGUIFont.symbolAtlas != null)
			{
				return nGUIFont.symbolAtlas != nGUIFont.atlas;
			}
			return false;
		}
	}
	...
	protected override void OnInit()
	{
		...
		if (mChanged || shouldBeProcessed)
		{
			ProcessAndRequest();
		}
		if (separateSymbols && mLabSym == null)
		{
			CreateSymbolLabel();
		}
	}
	...
	private void CreateSymbolLabel()
	{
		if (mLabSym == null)
		{
			GameObject gameObject = base.gameObject.AddChild(undo: false);
			gameObject.SetActive(value: false);
			gameObject.name = "Label Symbols";
			mLabSym = gameObject.AddComponent<UILabelSymbols>();
			mLabSym.gameObject.hideFlags = HideFlags.HideAndDontSave;
			mLabSym.label = this;
			mLabSym.pivot = pivot;
			mLabSym.depth = mSymbolDepth;
			mLabSym.leftAnchor.Set(base.transform, 0f, 0f);
			mLabSym.rightAnchor.Set(base.transform, 1f, 0f);
			mLabSym.topAnchor.Set(base.transform, 1f, 0f);
			mLabSym.bottomAnchor.Set(base.transform, 0f, 0f);
	...
	protected override void OnDisable()
	{
		...
		if (mLabSym != null)
		{
			GameObject gameObject = mLabSym.gameObject;
	...
	public void ProcessText(bool legacyMode = false, bool full = true)
	{
		...
		if (mFinalFontSize > 0)
		{
			...
			for (num3 = mFinalFontSize; num3 > 0; num3--)
			{
				...
				if (mOverflow == Overflow.ResizeFreely)
				{
					mCalculatedSize = NGUIText.CalculatePrintedSize(mProcessedText, prepare: false);
	...
	public void Fill(List<Vector3> verts, List<Vector2> uvs, List<Color> cols, List<Vector3> symbolVerts, List<Vector2> symbolUVs, List<Color> symbolCols)
	{
		...
		{
			int i = num2;
			for (int count2 = symbolVerts.Count; i < count2; i++)
			{
				Vector3 value = symbolVerts[i];
				value.x += vector.x;
				value.y += vector.y;
				symbolVerts[i] = value;
			}
		...
		if (effectStyle != 0)
		{
			...
			int num3 = symbolVerts?.Count ?? 0;
			vector.x = mEffectDistance.x;
			vector.y = mEffectDistance.y;
			ApplyShadow(verts, uvs, cols, num, count3, vector.x, 0f - vector.y);
			if (symbolVerts != null)
			{
				ApplyShadow(symbolVerts, symbolUVs, symbolCols, start, num3, vector.x, 0f - vector.y);
			}
			if (effectStyle == Effect.Outline || effectStyle == Effect.Outline8)
			{
				...
				count3 = verts.Count;
				start = num3;
				num3 = symbolVerts?.Count ?? 0;
				ApplyShadow(verts, uvs, cols, num, count3, 0f - vector.x, vector.y);
				if (symbolVerts != null)
				{
					ApplyShadow(symbolVerts, symbolUVs, symbolCols, start, num3, 0f - vector.x, vector.y);
				}
				num = count3;
				count3 = verts.Count;
				start = num3;
				num3 = symbolVerts?.Count ?? 0;
				ApplyShadow(verts, uvs, cols, num, count3, vector.x, vector.y);
				if (symbolVerts != null)
				{
					ApplyShadow(symbolVerts, symbolUVs, symbolCols, start, num3, vector.x, vector.y);
				}
				num = count3;
				count3 = verts.Count;
				start = num3;
				num3 = symbolVerts?.Count ?? 0;
				ApplyShadow(verts, uvs, cols, num, count3, 0f - vector.x, 0f - vector.y);
				if (symbolVerts != null)
				{
					ApplyShadow(symbolVerts, symbolUVs, symbolCols, start, num3, 0f - vector.x, 0f - vector.y);
				}
				if (effectStyle == Effect.Outline8)
				{
					...
					start = num3;
					num3 = symbolVerts?.Count ?? 0;
					ApplyShadow(verts, uvs, cols, num, count3, 0f - vector.x, 0f);
					if (symbolVerts != null)
					{
						ApplyShadow(symbolVerts, symbolUVs, symbolCols, start, num3, 0f - vector.x, 0f);
					}
					num = count3;
					count3 = verts.Count;
					start = num3;
					num3 = symbolVerts?.Count ?? 0;
					ApplyShadow(verts, uvs, cols, num, count3, vector.x, 0f);
					if (symbolVerts != null)
					{
						ApplyShadow(symbolVerts, symbolUVs, symbolCols, start, num3, vector.x, 0f);
					}
					num = count3;
					count3 = verts.Count;
					start = num3;
					num3 = symbolVerts?.Count ?? 0;
					ApplyShadow(verts, uvs, cols, num, count3, 0f, vector.y);
					if (symbolVerts != null)
					{
						ApplyShadow(symbolVerts, symbolUVs, symbolCols, start, num3, 0f, vector.y);
					}
					num = count3;
					count3 = verts.Count;
					start = num3;
					num3 = symbolVerts?.Count ?? 0;
					ApplyShadow(verts, uvs, cols, num, count3, 0f, 0f - vector.y);
					if (symbolVerts != null)
					{
						ApplyShadow(symbolVerts, symbolUVs, symbolCols, start, num3, 0f, 0f - vector.y);
					}
				}
			}
		}
		...
		{
			int j = 0;
			for (int count4 = cols.Count; j < count4; j++)
			{
				if (cols[j].r == -1f)
				{
					cols[j] = Color.white;
			...
			{
				int k = 0;
				for (int count5 = symbolCols.Count; k < count5; k++)
				{
					if (symbolCols[k].r == -1f)
					{
						symbolCols[k] = Color.white;
					}
				}
			}
		}
	...
	public void UpdateNGUIText()
	{
		...
		INGUIFont finalFont = this.font;
		if (finalFont != null)
		{
			NGUIText.spaceWidth = finalFont.spaceWidth;
		}
		...
		else if (finalFont != null)
		{
			...
			NGUIText.fontScale = ((finalFont != null) ? ((float)mFontSize / (float)finalFont.defaultSize * mScale) : mScale);
		}
		else
		{
			NGUIText.fontScale = mScale;
		}
		NGUIText.nguiFont = finalFont;
