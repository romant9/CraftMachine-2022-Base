...
public class UIFont : MonoBehaviour, INGUIFont
{
	...
	public NGUIFontType type
	{
		get
		{
			if (replacement != null)
			{
				return NGUIFontType.Reference;
			}
			if (dynamicFont != null)
			{
				return NGUIFontType.Dynamic;
			}
	...
	public INGUIAtlas symbolAtlas => atlas;
	...
	public int spaceWidth
	{
		get
		{
			return replacement?.spaceWidth ?? 0;
		}
		set
		{
			INGUIFont iNGUIFont = replacement;
	...
	public INGUIFont replacement
	{
		...
		set
		{
			...
			if (iNGUIFont == this as INGUIAtlas)
			{
				iNGUIFont = null;
			}
			if (mReplacement as INGUIFont != iNGUIFont)
			{
				if (iNGUIFont != null && iNGUIFont.replacement == this as INGUIAtlas)
				{
	...
	public void MarkAsChanged()
	{
		...
		for (int num = array.Length; i < num; i++)
		{
			...
			if (uILabel.enabled && NGUITools.GetActive(uILabel.gameObject) && NGUITools.CheckIfRelated(this, uILabel.font))
			{
				INGUIFont font = uILabel.font;
				uILabel.font = null;
				uILabel.font = font;
			}
		}
