...
public class UIPanel : UIRect
{
	...
	public override float CalculateFinalAlpha(int frameID)
	{
		if (mAlphaFrameID != frameID)
		{
			...
			finalAlpha = ((base.parent != null) ? uIRect.CalculateFinalAlpha(frameID) : 1f);
	...
	private void FillAllDrawCalls()
	{
		...
		if (!string.IsNullOrEmpty(mAlphaProp))
		{
			if (mOnRender == null)
			{
				mOnRender = OnSetAlphaProp;
			}
			else
			{
				mOnRender = (UIDrawCall.OnRenderCallback)Delegate.Combine(mOnRender, new UIDrawCall.OnRenderCallback(OnSetAlphaProp));
			}
		}
		int j = 0;
	...
	private void OnSetAlphaProp(Material mat)
	{
		if (mat != null)
		{
			mat.SetFloat(mAlphaProp, mAlpha);
		}
	}
