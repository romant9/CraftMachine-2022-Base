...
//#endif
	...
	protected void Init ()
	{
		if (mDoInit && label != null)
		{
			...
			if (label.alignment == NGUIText.Alignment.Justified)
			{
				...
				Debug.LogWarning("Input fields using labels with justified alignment are not supported at this time");
			}
	...
	protected void OnSelectEvent()
	{
		...
		selection = this;
//#if !MOBILE
//        if (!GetComponent<UIInputOnGUI>()) gameObject.AddComponent<UIInputOnGUI>();
//#endif
		this.OnSelected?.Invoke(selected: true);
		if (mDoInit) Init();
	...
	protected void OnDeselectEvent()
	{
		...
		selection = null;
		this.OnSelected?.Invoke(selected: false);
//#if !MOBILE
//		if (GetComponent<UIInputOnGUI>()) Destroy(GetComponent<UIInputOnGUI>());
//#endif
		UpdateLabel();
