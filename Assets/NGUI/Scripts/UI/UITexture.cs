...
public class UITexture : UIBasicSprite
{
	...
	public override Shader shader
	{
		get
		{
			if (mMat != null)
			{
				if (Application.isPlaying && ParentPanel) // && ParentPanel.clipping == UIDrawCall.Clipping.SoftClip) || mMat.name.ToLower().Contains("glow")))
				{
					if (NGUIManager.IsShaderNameContains_Jump(mMat.shader))
					{
						mMat.SetInt("_ColorToOpacity", 1);
						mShader = NGUIManager.TranspMaskedShader;
						return mShader;
					}
					if (NGUIManager.IsShaderNameContains_Transp(mMat.shader))
					{
						mShader = NGUIManager.TranspColoredShader;
						return mShader;
					}
				}
