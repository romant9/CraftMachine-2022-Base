...
namespace GoogleSheetsToUnity
{
    ...
    public class GSTU_SpreadsheetResponce
    {
        ...
        public string StartCell()
        {
            ...
            //Debug.Log("valueRange.range " + valueRange.range);

            if (string.IsNullOrEmpty(valueRange.range))
            {
                return string.Empty;
            }
    ...
    public class GstuSpreadSheet
    {
        ...
        {
            if (data == null || data.valueRange == null || string.IsNullOrEmpty(data.valueRange.range))
            {
                if (DataManager.Instance != null)
                {
                    DebugTWD.LogWarning("There is error of updating player's data. Please reconnect");
                    MyTools.UpdateLogPanel("There is error of updating player's data. Please reconnect");
                }
                return;
            }

            string startCell = data.StartCell();

            if (string.IsNullOrEmpty(startCell)) 
            {
                if (DataManager.Instance != null)
                {
                    DebugTWD.LogWarning("Spreadsheet Responce is null");
                    MyTools.UpdateLogPanel("There is error of updating player's data. Please reconnect");
                }
                return; 
            }

            string startColumn = Regex.Replace(startCell, "[^a-zA-Z]", "");
            int startRow = int.Parse(Regex.Replace(startCell, "[^0-9]", ""));
