...
namespace GoogleSheetsToUnity
{
	public class GoogleAuthrisationHelper : MonoBehaviour
	{
		...
		static void GetAuthComplete(string authToken)
		{
			...
			DebugTWD.Log(authToken);
			DebugTWD.Log("Auth Token = " + authToken);
		...
		static void ListenerCallback(IAsyncResult result)
		{
			if (_httpListener != null)
			{
				...
				{
					DebugTWD.Log(exception.Message + " : " + exception.StackTrace); // Just in case...
				}
			}
		}
		...
		/// </summary>
		/// <returns></returns>
		public static IEnumerator CheckForRefreshOfToken(bool isRefreshImmediate)
		{
			if (DateTime.Now > SpreadsheetManager.Config.gdr.nextRefreshTime || isRefreshImmediate)
			{
				DebugTWD.Log("Refreshing Google Token");
				...
				using (UnityWebRequest request = UnityWebRequest.Post("https://www.googleapis.com/oauth2/v4/token", f))
				{
					...
					if (newGdr == null || string.IsNullOrEmpty(newGdr.access_token))
					{
						//CraftSettings.Instance.IsGoogleSheetConnected = false;
						//CraftSettings.Instance.CheckInternetStatus();
						DebugTWD.Log("Internet is False");
						yield break;
					}
					else
					{
						//CraftSettings.Instance.IsGoogleSheetConnected = true;
						DebugTWD.Log("Internet is True");
					}
					...
					DebugTWD.Log("nextRefreshTime is " + SpreadsheetManager.Config.gdr.nextRefreshTime.ToString(UserPrefsKeys.TimeFormat));
				}
			}
		}
	}
