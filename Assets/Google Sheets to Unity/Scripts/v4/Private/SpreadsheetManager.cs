...
namespace GoogleSheetsToUnity
{
    ...
    {   
        ...
        static IEnumerator CheckForRefreshToken()
        {
            ...
            {
                yield return new Task(GoogleAuthrisationHelper.CheckForRefreshOfToken(false));
            }
#if UNITY_EDITOR
            else
            {
                yield return EditorCoroutineRunner.StartCoroutine(GoogleAuthrisationHelper.CheckForRefreshOfToken(false));
            }
#endif
        }    
        ...
        public static void Read(GSTU_Search search, UnityAction<GstuSpreadSheet> callback, bool containsMergedCells = false)
        {
            ...
            //sb.Append("https://sheets.googleapis.com/v4/spreadsheets");
            //sb.Append("/" + "1_LO2NMbiMd1izu0pu4EUZ3D1O5kME2PpLCl2p6Ay1T8");
            //sb.Append("/values");
            //sb.Append("/" + "Stats" + "!" + search.startCell + ":" + search.endCell);
            //sb.Append("?access_token=" + Config.gdr.access_token);

            //sb.Append("https://sheets.googleapis.com/v4/spreadsheets");
            //sb.Append("/" + "1_LO2NMbiMd1izu0pu4EUZ3D1O5kME2PpLCl2p6Ay1T8");
            //sb.Append("/values");
            //sb.Append("/" + "List12" + "!" + search.startCell + ":" + search.endCell);
            //sb.Append("?access_token=" + Config.gdr.access_token);
        ...
        static IEnumerator Read(UnityWebRequest request, GSTU_Search search, bool containsMergedCells, UnityAction<GstuSpreadSheet> callback)
        {
            ...
            using (request)
            {
                ...
                {
                    DebugTWD.LogWarning("Unable to Retreive data from google sheets");
                    MyTools.UpdateLogPanel("Unable to Retreive data from google sheets. Please reconnect manually");
                    IsError = true;
                    yield break;
                }
                ...
                if (rawData == null || rawData.values.Count == 0) 
                {
                    DebugTWD.LogWarning("Unable to Retreive data from google sheets");
                    MyTools.UpdateLogPanel("Unable to Retreive data from google sheets. Please reconnect");
                    IsError = true;
                    yield break;
                }
                ...
                if (string.IsNullOrEmpty(responce.valueRange.range))
                {                  
                    DebugTWD.LogWarning("There is error of updating player's data. Please reconnect");
                    MyTools.UpdateLogPanel("There is error of updating player's data. Please reconnect manually");
                    IsError = true;
                    yield break;
                }
