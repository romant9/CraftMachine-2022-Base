...
namespace GoogleSheetsToUnity
{
    ...
    {      
        ...
        public static void ReadPublicSpreadsheet(GSTU_Search searchDetails, OnSpreedSheetLoaded callback)
        {
            ...
            {
                DebugTWD.Log("Missing API Key, please enter this in the confie settings");
                return;
            }
        ...
        static IEnumerator Read(WWW www, string titleColumn, int titleRow, OnSpreedSheetLoaded callback)
        {
            ...
            if (rawData == null && rawData.values.Count == 0)
            {
                if (Application.isPlaying)
                {
                    new Task(Read(new WWW(www.url), titleColumn, titleRow, callback));
                }
#if UNITY_EDITOR
                else
                {
                    EditorCoroutineRunner.StartCoroutine(Read(new WWW(www.url), titleColumn, titleRow, callback));
                }
#endif
                yield break;
            }
