...
namespace GoogleSheetsToUnity.ThirdPary
{
    public class EditorCoroutineRunner
    {
        ...
        public static EditorCoroutine StartCoroutineWithUI(IEnumerator coroutine, string title, bool isCancelable = false)
        {
            ...
            {
                DebugTWD.LogError("EditorCoroutineRunner only supports running one coroutine that draws a GUI! [" + title + "]");
                return null;
            }
        ...
        public void Tick()
        {
            if (coroutine != null)
            {
                ...
                if (current != null)
                {
                    ...
                    {
                        // UNSUPPORTED
                        DebugTWD.LogError("Nested Coroutines started by Unity's defaut StartCoroutine method are not supported in editor! please use EditorCoroutineRunner.Start instead. Canceling.");
                        canceled = true;
                    }
                    else
                    {
                        // UNSUPPORTED
                        DebugTWD.LogError("Unsupported yield (" + currentType + ") in editor coroutine!! Canceling.");
                        canceled = true;
                    }
                }
                lastUpdateTime = now;
